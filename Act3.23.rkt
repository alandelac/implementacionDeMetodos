...
(define (imprime lista p2)
  ...
  (define (despliega lista p2)
    (if (symbol? (second lista))
        (display (string-append "<span style = \"" "color: " (color tipo-color (first lista)) "\">" (symbol->string (second lista)) "</span> ") p2)
...
(define (recorre file1 file2)
  ...
  (display (string-append "<div style = \"" "color: " (color tipo-color "dato") "\"> Cantidad de Tokens:</div> ") p2)
  (display cantidad p2)
  (display "</html>" p2)
