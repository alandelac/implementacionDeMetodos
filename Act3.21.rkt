...
(define (imprime lista p2)
  ...
  (define (despliega lista p2)
    (display (string-append "<span style = \"" "color: " (color tipo-color (first lista)) "\">" (slist->string (list(second lista))) "</div> ") p2)
    1)
...
(define (convierte atomo p1)
  ...
  ;(list (regexp-match? #rx"=|!|\\+|\\*|/|-|%|>|<|\\(|\\)|\\[|\\]|{|}|;|," atomo) atomo))
  ;Checa si es un número y su tipo
      ...
      (cond
        [(regexp-match? #rx"for|while" p1) (list "ciclo" atomo)]
         [(regexp-match? #rx"if|>|<|==|<=|>=" p1) (list "condi" atomo)]
        [(regexp-match? #rx"=|!|\\+|\\*|/|-|%" p1) (list "simb ari" atomo)]
         [(regexp-match? #rx"int|float|double|long|byte|boolean|string|switch|case|class|private|break|return|#include|public|char|cin|cout|endl|void|using|namespace" p1) (list "palabra" atomo)]
